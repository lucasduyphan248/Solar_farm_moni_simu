...
def read_all_scale_factors_from_slave(modbus_client: ModbusClient):
    ...
    for sf_name, sf_data in sunssf_registers.items():
        ...
        # sunssf_registers[name]["default"] = new_valuecc
...
def read_all_data_needed_from_slave(modbus_client: ModbusClient, sf: dict=None):
    modbus_data = list()
...
# def SIMU_read_data_from_slave(server_ip, port, slave_id):
...
#     }
#     return modbus_data, modbus_data_dict, units
# def SIMU_read_attributes_from_slave(server_ip, port, slave_id=1):
#     modbus_data =[random.random() for _ in range(5)]
#     modbus_data_dict = {
...
def read_data_from_slave(server_ip, port=502, slave_id=1):
    ...
    return read_all_data_needed_from_slave(modbus_client, sf)
    ...
    try:    
        while True:
            ...
            for port, slave_id, device in zip(server_ports, slave_ids, list_devices):
                ...
                giatri = modbus_client.read_holding_registers(40001, 2)
                print(giatri)
                for entry_name, entry_info in chain(kaco_common_registry_map.items(), kaco_tx3_registry_map.items()):
                    # start_addr = int(entry_info.get('start_addr', 0))
                    # print(start_addr)
                    # giatri = modbus_client.read_holding_registers(40076, 1)
                    # print(f"gia tri: {giatri}")
                    modbus_data, modbus_data_dict, units = read_data_from_slave(server_ip, port, slave_id)
                    if modbus_data:
            ...
            print(read_all_scale_factors_from_slave(modbus_client))
            time.sleep(50)
